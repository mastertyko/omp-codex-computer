...
5. Locator actions require an unambiguous target: exactly one match, or exactly one visible match among duplicates. On `element_not_found`, `ambiguous_locator`, or a failed locator resolution no action was performed: observe the page, refine the target (for example add a role name or switch to a label), and try again. Do not blind-retry the same locator.

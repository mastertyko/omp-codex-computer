...
NPM releases are automated from `main` by the `Release` GitHub Actions workflow. The workflow reads `package.json` version, skips when tag `v<version>` already exists, and otherwise:
...
The `Publish npm package` workflow remains available for a manual publish from a GitHub release or workflow dispatch.

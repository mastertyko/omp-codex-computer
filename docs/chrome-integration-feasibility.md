...
## Live verification 2026-09-02 (Codex 0.151.0, plugin 26.831.21537)

Codex updated from 0.149.0 to 0.151.0 and the bundled Chrome plugin from `26.818.61809` to `26.831.21537` (a sibling `browser` plugin for the in-app browser now ships the same client bundle; the gate still selects only `chrome@openai-bundled`). The gate behaved as designed: the plugin contract check passed, and the app-server check failed closed with `unsupported_app_server_version`. Review before widening the built-in allowlist:

1. An exact schema diff (`codex app-server generate-json-schema` from a temp install of `@openai/codex@0.149.0` versus the installed 0.151.0) over the transitive closure of the used methods — `initialize`, `plugin/list`, `mcpServerStatus/list` (`ListMcpServerStatusParams`/`Response`), `thread/start`, `mcpServer/tool/call`, and the standalone `McpServerElicitationRequestParams`/`Response` — found the unresolved request/response root schemas, JSON-RPC envelopes, and server-request method set identical. Eleven transitively referenced definitions changed, all additively: optional `McpServerStatus.runtimeStatus` (nullable connection state) and `Thread.historyMode` (`legacy`/`paginated`, default `legacy`) in the two response objects this extension reads, plus `TurnError.misalignment`, a `CodexErrorInfo` enum variant, a `ThreadItem` union variant, and collab/sub-agent enum variants under `Thread.turns`, which it never reads. Nothing was removed, retyped, or made required, and every consumed field is unchanged. New server notifications (`mcpServer/event/stream/notification`, `thread/realtime/item/*`) are logged and ignored by `AppServerClient`. Fixtures mirroring the live 0.151.0 `mcpServerStatus/list` and `thread/start` payloads were added to the Sky-route, Chrome-gate, and thread-manager tests.
2. A static review of the new `browser-client.mjs` confirmed the single `setupBrowserRuntime` export (still bootstrapping through `nodeRepl.rpc("browser", …)`), `browsers.get`, `nameSession`, `tabs.new`, tab `goto`/`back`/`forward`/`reload`/`close`, `tab.playwright` locator factories, `domSnapshot`, and locator `first`/`waitFor({ state, timeoutMs })`/`count`/`nth`/`isVisible`/`click`/`fill`/`press`/`selectOption({ label })`/`setChecked` accepting `timeoutMs`.
3. A live `ChromeRuntime` probe with a probe-only trust override ran open → observe → reload → role-locator click → back → missing locator (`element_not_found`, benign) → observe → close → cleanup green; `0.151.0` was then added to the built-in allowlist and the default-wired status probe reported `ready`.
4. Native Computer Use was unaffected: the Sky/node_repl route bootstrapped against Computer Use plugin `1.0.1000926`, and live `list_apps`/`get_app_state` returned real data.
